...
func makeEmeraldRouter(dbPool *pgxpool.Pool, conn *grpc.ClientConn) *chi.Mux {
	...
	r.Get("/home/latest_blocks", fallible(func(w http.ResponseWriter, r *http.Request) error {
		...
		if err := dbPool.BeginFunc(r.Context(), func(dbTx pgx.Tx) error {
			var batch pgx.Batch
			batch.Queue("SELECT height, b_hash, num_transactions, gas_used, size FROM block_extra WHERE chain_alias = $1 ORDER BY height DESC LIMIT $2", chainAlias, latestBlocksCount)
			...
			for blockExtraRows.Next() {
				var br BlockRow
				if err = blockExtraRows.Scan(&br.Height, &br.Hash, &br.NumTransactions, &br.GasUsed, &br.SizeBytes); err != nil {
					return err
				}
	...
	r.Get("/home/latest_transactions", fallible(func(w http.ResponseWriter, r *http.Request) error {
		...
		for height, trs := range txsByBlock {
			...
			for _, tr := range trs {
				...
				if tx != nil {
					tr.FeeAmount = tx.AuthInfo.Fee.Amount.Amount.String()
