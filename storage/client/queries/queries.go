...
const (
	...
	RuntimeEvents = `
		...
		LEFT JOIN chain.runtime_events_related_accounts as acc ON
			evs.runtime = acc.runtime AND
			evs.round = acc.round AND
			evs.tx_index = acc.tx_index AND
			AND ($7::text IS NOT NULL AND acc.related_account = $7::text)
		WHERE
			(evs.runtime = $1) AND
			...
			($7::text IS NULL OR acc.related_account IS NOT NULL) AND
			($8::text IS NULL OR (
