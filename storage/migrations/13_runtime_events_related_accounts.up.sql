...
WITH ranked_events AS (
  SELECT ctid,
         row_number() OVER (PARTITION BY runtime, round ORDER BY timestamp ASC) - 1 AS event_idx
...
INSERT INTO chain.runtime_events_related_accounts (runtime, round, event_index, tx_index, type, account_address)
    SELECT runtime, round, event_index, tx_index, type, unnest(related_accounts)
    FROM chain.runtime_events
