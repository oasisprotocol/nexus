		...
		if tx != nil { //nolint:nestif
			...
			if fail := txr.Result.Failed; fail != nil { //nolint:gocritic
				...
				if len(fail.Message) > 0 {
					// The message is not guaranteed to be valid UTF-8 as it can include substrings generated by the smart contract.
					// Postgres requires valid UTF-8 with no 0x00, so we sanitize the message.
					sanitizedMsg := strings.ToValidUTF8(strings.ReplaceAll(fail.Message, "\x00", "?"), "?")
					blockTransactionData.Error.Message = &sanitizedMsg
				}
