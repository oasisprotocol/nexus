...
const (
	...
	RuntimeRoflInstanceTransactions = `
        -- Fetch latest round, so we know up to which round we have processed.
        WITH latest AS (
            SELECT round
        ...
        SELECT
            s.round, s.tx_index, t.method, t.likely_native_transfer, latest.round
        FROM
            chain.runtime_transaction_signers AS s
            JOIN chain.runtime_transactions AS t ON
        ...
        -- it seems fine to skip/miss some transactions.
        LIMIT 1000`
