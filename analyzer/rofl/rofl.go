...
func NewAnalyzer(
	...
	logger = logger.With("analyzer", roflAnalyzerPrefix+runtime)
	p := &processor{
		runtime: runtime,
		source:  sourceClient,
		target:  target,
	...
	return item.NewAnalyzer(
		roflAnalyzerPrefix+string(runtime),
		cfg,
		p,
		target,
		logger,
...
type Item struct {
	...
	// TODO: Could track instance refreshes separately, to avoid app refresh when only instances are updated.
...
func (p *processor) queueRoflAppRefresh(ctx context.Context, batch *storage.QueryBatch, round uint64, id nodeapi.AppID) error {
	...
	if err != nil {
		...
		if module == "rofl" && code == 2 { // "unknown application"
			// App was deleted.
			batch.Queue(
				queries.RuntimeRoflAppDeleteInstances,
				p.runtime,
				id.String(),
			)
			batch.Queue(
				queries.RuntimeRoflAppDelete,
				p.runtime,
				id.String(),
			)
			return nil
...
func (p *processor) queueRoflAppInstancesRefresh(ctx context.Context, batch *storage.QueryBatch, round uint64, id nodeapi.AppID) error {
	...
	for _, instance := range instances {
		...
		for _, key := range instance.ExtraKeys {
			extraKeys = append(extraKeys, key.String())
		}
